    ...
    <>
      ...
      <div className="navbar absolute h-24 w-full bg-gradient-to-b from-black text-white flex justify-between items-center pr-14 z-[999]">
        ...
        <div className="text-3xl h-16  p-1 flex items-center gap-6 rounded-full">
          <div className="active:scale-95 text-white text-xl font-bold  px-8 py-2 rounded-xl bg-gradient-to-r from-purple-800 via-purple-500 to-purple-800">
          ...
          {dropdown && (
            <div className={`bg-zinc-700 text-white text-center ${gptview?"translate-x-[60%]":"translate-x-[100%]"} translate-y-28 rounded h-40 w-40 outline outline-1 absolute pt-4`}>
              <h1
              ...
              {gptview?<select onChange={handleLang} className="bg-transparent text-lg font-semibold">
                {langOpt.map((elem,index)=>{
               return <option   value={elem.name} className="bg-zinc-700 ">{elem.name}</option>
                })}
              </select>:null}
